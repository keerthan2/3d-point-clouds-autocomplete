...
def main(config: dict):
    ...
    if run_mode == 'training':
        ...
        for epoch in range(latest_epoch + 1, config['training']['max_epoch'] + 1):
            ...
            train_losses.append(np.array([epoch_loss_all.detach().cpu().numpy(), epoch_loss_r.detach().cpu().numpy(), epoch_loss_kld.detach().cpu().numpy()]))
            ...
            if (epoch % config['training']['state_save_frequency'] == 0) \
                    and epoch > config['training'].get('min_save_epoch', 0):
